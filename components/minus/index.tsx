...
export default function Index() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const myText = `Résumé d'observation:
Examen du ${age}e mois. ${
      entries.AgeReel ? `Âge civil ${entries.AgeReel}. ` : ""
    } ${entries.AgeCorrige ? `Âge corrigé ${entries.AgeCorrige}. ` : ""}
Constantes: poids ${entries.Poids} kg, taille ${entries.Taille} cm (IMC ${
      entries.IMC
    }), PC ${entries.PC} cm
Développement psycho-moteur:
- ${keys[0]}:${
      psychomoteur[keys[0]].neFaitPas.length > 0
        ? psychomoteur[keys[0]].neFaitPas.map((e: any) => ` ${e} (ne fait pas)`)
        : " RAS"
    }
- ${keys[1]}:${
      psychomoteur[keys[1]].neFaitPas.length > 0
        ? psychomoteur[keys[1]].neFaitPas.map((e: any) => ` ${e} (ne fait pas)`)
        : " RAS"
    }
- ${keys[2]}:${
      psychomoteur[keys[2]].neFaitPas.length > 0
        ? psychomoteur[keys[2]].neFaitPas.map((e: any) => ` ${e} (ne fait pas)`)
        : " RAS"
    }
- ${keys[3]}:${
      psychomoteur[keys[3]].neFaitPas.length > 0
        ? psychomoteur[keys[3]].neFaitPas.map((e: any) => ` ${e} (ne fait pas)`)
        : " RAS"
    }

${
  psychomoteur.retard.length > 0
    ? psychomoteur.retard.map((e: any) => ` ${e} (RETARD PSYCHOMOTEUR)`)
    : ""
}
`;
